...
namespace model {
	...
	//TODO il faut encore gérer la simulation du prochain mur et
	//le fait que les murs sont sur les cases impaires et pas les pions
	bool Model::peutPlacerMur(int pionX, int pionY, int murX, int murY, bool origin) {
		if (taillePlateau % 2 != 1) {
			throw new std::invalid_argument("La taille doit être impaire");
		}
		...
		while (!pile.empty()) {
			...
			vector<array<int,2>> succs = getSuccessors(coord);
	...
	vector<array<int, 2>> Model::getSuccessors(const array<int, 2> coord) {
		...
		//s'il y a encore de la place à gauche et pas de mur
		if (coord.at(0) > 0 && tablePosition[coord.at(0) - 1][coord.at(1)] != MUR) {
			array<int, 2> push = { coord.at(0) - 1, coord.at(1) };
			ret.push_back(push);
		}
		//s'il y a encore de la place à droite et pas de mur
		if (coord.at(0) < taillePlateau - 1 && tablePosition[coord.at(0) + 1][coord.at(1)] != MUR) {
			array<int, 2> push = { coord.at(0) + 1, coord.at(1) + 1 };
			ret.push_back(push);
		}
		//s'il y a encore de la place en haut et pas de mur
		if (coord.at(1) > 0 && tablePosition[coord.at(0)][coord.at(1) - 1] != MUR) {
			array<int, 2> push = { coord.at(0), coord.at(1) - 1 };
			ret.push_back(push);
		}
		//s'il y a encore de la place en bas et pas de mur
		if (coord.at(0) < taillePlateau - 1 && tablePosition[coord.at(0)][coord.at(1) + 1] != MUR) {
			array<int, 2> push = { coord.at(0), coord.at(1) + 1 };
			ret.push_back(push);
		}
		return ret;
